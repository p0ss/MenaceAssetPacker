...
public class CombinedArmsPlugin : IModpackPlugin
{
    public static CombinedArmsPlugin Instance;
    ...
    // TileScore value-type handling — if TileScore is a struct, it's inlined
    // in dictionary entries and must NOT be read as a pointer.
    private static bool _tileScoreIsValueType;
    private static int _tileScoreInlineUtilityOffset;

    // Diagnostic: set true after first successful PostProcessTileScores completion
    private static bool _diagTileScoresDone;
    ...
    private void ResolveAllOffsets()
    {
        ...
        if (tileScoreClass != IntPtr.Zero)
        {
            ...
            try
            {
                _tileScoreIsValueType = IL2CPP.il2cpp_class_is_valuetype(tileScoreClass);
                Log?.Msg($"[CombinedArms] TileScore isValueType={_tileScoreIsValueType}");
                if (_tileScoreIsValueType && _off_TileScore_UtilityScore != 0)
                {
                    // For inlined value types, il2cpp_field_get_offset includes the boxed
                    // object header (2*IntPtr). Subtract header to get raw struct offset.
                    _tileScoreInlineUtilityOffset = (int)_off_TileScore_UtilityScore - IntPtr.Size * 2;
                    Log?.Msg($"[CombinedArms] TileScore inline UtilityScore offset=0x{_tileScoreInlineUtilityOffset:X}");
                }
            }
            catch (Exception ex)
            {
                Log?.Warning($"[CombinedArms] il2cpp_class_is_valuetype check failed: {ex.Message} — assuming reference type");
            }
        }
    ...
    private void PatchOnTurnStart(Assembly asm)
    {
        ...
        try
        {
            _harmony.Patch(method,
                postfix: new HarmonyMethod(typeof(CombinedArmsPlugin),
    ...
    public static void OnTurnStart_Postfix(object __instance)
    {
        ...
        {
            CrashDiag($"OnTurnStart EXCEPTION: {ex.Message}");
    ...
    public static void GetScoreMult_Postfix(object __instance, ref float __result)
    {
        ...
        try
        {
            ...
            CrashDiag($"ScoreMult#{callNum}: faction={factionIndex} original={__result:F3}");
            ...
            CrashDiag($"ScoreMult#{callNum}: final={__result:F3}");
        }
        catch (Exception ex)
        {
            CrashDiag($"ScoreMult#{callNum} EXCEPTION: {ex.Message}");
    ...
    public static void Execute_Postfix(object __instance, bool __result)
    {
        ...
        try
        {
            ...
            CrashDiag($"Execute#{callNum}: faction={factionIndex} result={__result} actor=0x{actorPtr.ToInt64():X} behavior=0x{activeBehaviorPtr.ToInt64():X}");
            ...
            if (activeBehaviorPtr != IntPtr.Zero && IsAttackBehavior(activeBehaviorPtr))
            {
                ...
                CrashDiag($"Execute#{callNum}: attack goal={action.BehaviorGoal} targetTile=0x{targetTilePtr.ToInt64():X}");
            ...
            {
                CrashDiag($"Execute#{callNum}: getting actor position...");
                bool alive = IsNativeObjectAlive(actorPtr);
                CrashDiag($"Execute#{callNum}: actor alive={alive}");
                ...
                if (alive)
                {
                    try
                    {
                        ...
                        CrashDiag($"Execute#{callNum}: pos=({tilePos.x},{tilePos.z})");
                    }
                    catch (Exception posEx)
                    {
                        CrashDiag($"Execute#{callNum}: GetActorTilePosition EXCEPTION: {posEx.Message}");
                    }
                }
            }

            state.CompletedActions.Add(action);
            CrashDiag($"Execute#{callNum}: done, total actions={state.CompletedActions.Count}");
        ...
        {
            CrashDiag($"Execute#{callNum} EXCEPTION: {ex.Message}");
    ...
    private static int _pptsCallCount;
    ...
    public static void PostProcessTileScores_Postfix(object __instance)
    {
        ...
        int callNum = ++_pptsCallCount;
        bool dryRun = config.DryRunTileScores;
        ...
        try
        {
            if (!_loggedFirstTileScores)
            {
                ...
                Log?.Msg($"[CombinedArms]   wantCoF={wantCoF} wantDepth={wantDepth} tileScoreIsValueType={_tileScoreIsValueType} dryRun={dryRun}");
            }

            if (__instance is not Il2CppObjectBase agentObj) return;

            IntPtr agentPtr = agentObj.Pointer;
            CrashDiag($"PPTS#{callNum} step 1: reading faction (dryRun={dryRun})");
            ...
            CrashDiag($"PPTS#{callNum} step 2: tilesPtr=0x{tilesPtr.ToInt64():X} faction={factionIndex} allies={state.PreviousTurnAllyPositions.Count}");
            ...
            if (wantCoF && state.PreviousTurnAllyPositions.Count >= config.CenterOfForcesMinAllies)
            {
                ...
                CrashDiag($"PPTS#{callNum} step 3: CoF centroid=({centroidX:F1},{centroidZ:F1})");
                IterateTileScoresAndApplyCoF(tilesPtr, centroidX, centroidZ, config, callNum, dryRun);
                CrashDiag($"PPTS#{callNum} step 3b: CoF complete");
            ...
            if (wantDepth)
            {
                ...
                if (state.DepthCache.EnemyCentroids.Count > 0)
                {
                    ...
                    CrashDiag($"PPTS#{callNum} step 5: depth iteration band={band}");

                    IterateTileScoresAndApplyFormationDepth(
                        tilesPtr, state.DepthCache, band, config, callNum, dryRun);

                    CrashDiag($"PPTS#{callNum} step 5b: depth iteration complete");
            ...
            CrashDiag($"PPTS#{callNum} COMPLETED SUCCESSFULLY");
            Log?.Msg("[CombinedArms] PostProcessTileScores completed successfully");
        }
        catch (Exception ex)
        {
            CrashDiag($"PPTS#{callNum} EXCEPTION: {ex.Message}");
        ...
        int callNum, bool dryRun)
    {
        try
        {
            ...
            CrashDiag($"CoF#{callNum}: count={count} stride={entrySize}");
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (tilePtr == IntPtr.Zero) continue;

                IntPtr utilityAddr;
                if (_tileScoreIsValueType)
                {
                    utilityAddr = entryAddr + 8 + IntPtr.Size + _tileScoreInlineUtilityOffset;
                }
                else
                {
                    IntPtr tileScorePtr = Marshal.ReadIntPtr(entryAddr + 8 + IntPtr.Size);
                    if (tileScorePtr == IntPtr.Zero) continue;

                    // Log first few entries for diagnostics
                    if (modified < 3)
                        CrashDiag($"CoF#{callNum} [{i}]: tileScorePtr=0x{tileScorePtr.ToInt64():X} off=0x{_off_TileScore_UtilityScore:X}");

                    utilityAddr = tileScorePtr + (int)_off_TileScore_UtilityScore;
                }
                ...
                float currentUtility = ReadFloat(utilityAddr);
                float newUtility = currentUtility + bonus;

                if (modified < 3)
                    CrashDiag($"CoF#{callNum} [{i}]: read={currentUtility:F3} bonus={bonus:F3} new={newUtility:F3} addr=0x{utilityAddr.ToInt64():X} isNaN={float.IsNaN(currentUtility)}");

                if (!dryRun)
                    WriteFloat(utilityAddr, newUtility);

                modified++;
            }

            CrashDiag($"CoF#{callNum}: done, modified={modified} (dryRun={dryRun})");
        }
        catch (Exception ex)
        {
            CrashDiag($"CoF#{callNum} EXCEPTION: {ex.Message}");
    ...
    private static void TryComputeEnemyCentroids(FormationDepthCache cache, IntPtr factionPtr, CombinedArmsConfig config)
    {
        ...
        try
        {
            ...
            if (_cachedAIFactionKlass != IntPtr.Zero)
            {
                ...
                {
                    if (diag) CrashDiag("Centroids: faction is not AIFaction, skipping");
                    return;
                }
            }

            if (diag) CrashDiag("Centroids: reading opponents list");
            ...
            if (diag) CrashDiag($"Centroids: iterating {count} opponents");
        ...
        int callNum, bool dryRun)
    {
        try
        {
            ...
            CrashDiag($"Depth#{callNum}: count={count} stride={entrySize} band={band}");
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else
                {
                    ...
                    if (modified < 3)
                        CrashDiag($"Depth#{callNum} [{i}]: tileScorePtr=0x{tileScorePtr.ToInt64():X} off=0x{_off_TileScore_UtilityScore:X}");
                ...
                float currentUtility = ReadFloat(utilityAddr);
                float newUtility = currentUtility + delta;

                if (modified < 3)
                    CrashDiag($"Depth#{callNum} [{i}]: read={currentUtility:F3} delta={delta:F3} new={newUtility:F3} addr=0x{utilityAddr.ToInt64():X} isNaN={float.IsNaN(currentUtility)}");
            ...
            CrashDiag($"Depth#{callNum}: done, modified={modified} (dryRun={dryRun})");
        }
        catch (Exception ex)
        {
            CrashDiag($"Depth#{callNum} EXCEPTION: {ex.Message}");
    ...
    /// <summary>
    /// Crash-surviving diagnostic log. Appends to a file that is flushed on every call.
    /// MelonLogger may buffer output that is lost on native crashes — this survives.
    /// </summary>
    private static void CrashDiag(string step)
    {
        try
        {
            if (_diagPath == null)
            {
                _diagPath = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "CombinedArms_crash_diag.txt");
                System.IO.File.WriteAllText(_diagPath, $"[{DateTime.Now:HH:mm:ss.fff}] === SESSION START ===\n");
            }
            System.IO.File.AppendAllText(_diagPath, $"[{DateTime.Now:HH:mm:ss.fff}] {step}\n");
        }
        catch { }
    }
